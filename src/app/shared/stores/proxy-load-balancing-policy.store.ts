...
import { CommonStoreInitialState, ReverseProxyOptionDto } from '@shared/types';

const initialState: CommonStoreInitialState<ReverseProxyOptionDto[]> = {
  data: null,
  _loading: false,
  error: null,
  searchTerm: '',
...
export const ProxyLoadBalancingPolicyStore = signalStore(
  {
    providedIn: 'root',
  },
    ...
    const methods = {
      ...
      load: rxMethod<void>(
        pipe(
          ...
          switchMap(() =>
            reverseProxyApiService.getProxyLoadBalancingPolicies().pipe(
              tap((response) => {
                patchState(store, {
                  data: response ?? [],
                  _loading: false,
      ...
      patchData: (data: ReverseProxyOptionDto[]): void => {
